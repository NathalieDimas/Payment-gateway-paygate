...
class PayGate_Payment_Gateway extends WC_Payment_Gateway {

  // Configuracion del id, descripcion y otros valores de la pasarela de pago
  function __construct() {
    //ID global de nuestra pasarela de pago
    $this->id = "PayGate_payment_gateway";
    $this->method_title = __( "PayGate", 'PayGate-payment-gateway' );
    $this->method_description = __( "plugin de WooCommerce para realizar autorización y captura de pagos
    con tarjetas de crédito/débito", 'PayGate-payment-gateway' );
    $this->title = __( "PayGate", 'PayGate-payment-gateway' );
  ...
  public function init_form_fields() {
    $this->form_fields = array(
      'enabled' => array(
        ...
        'title'   => __( 'Activar / Desactivar', 'PayGate-payment-gateway' ),
        'label'   => __( 'Activar metodo de pago', 'PayGate-payment-gateway' ),
        'type'    => 'checkbox',
        'default' => 'no',
      ),
      'title' => array(
        'title'   => __( 'Título', 'PayGate-payment-gateway' ),
        'type'    => 'text',
        'desc_tip'  => __( 'Metodo de pago', 'PayGate-payment-gateway' ),
        'default' => __( 'PayGate', 'PayGate-payment-gateway' ),
      ),
      'description' => array(
        'title'   => __( 'Descripción', 'PayGate-payment-gateway' ),
        'type'    => 'textarea',
        'desc_tip'  => __( 'Paga con tarjeta de credito mediante PayGate.', 'PayGate-payment-gateway' ),
        'default' => __( 'Pague con seguridad usando su tarjeta de crédito.', 'PayGate-payment-gateway' ),
        'css'   => 'max-width:350px;'
      ),
      // INIT------> Prueba de authorize.net <------
      'api_login' => array(
        'title'    => __( 'API Login', 'PayGate-payment-gateway' ),
        'type'    => 'text',
        'desc_tip'  => __( 'Esta llave la provee Authorize.net al registrarse', 'PayGate-payment-gateway' ),
      ),
      'trans_key' => array(
        'title'    => __( 'Llave de transaccion', 'PayGate-payment-gateway' ),
        'type'    => 'password',
        'desc_tip'  => __( 'Esta llave la provee Authorize.net al registrarse', 'PayGate-payment-gateway' ),
      ),
      'environment' => array(
        'title'    => __( 'Modo de prueba por authorize.net', 'PayGate-payment-gateway' ),
        'label'    => __( 'Habilitar el modo de prueba', 'PayGate-payment-gateway' ),
        'type'    => 'checkbox',
        'description' => __( 'testea el plugin de pago con Authorize.net', 'PayGate-payment-gateway' ),
        'default'  => 'no',
      ),
      ...
      'key_id' => array(
        'title'   => __( 'Key id', 'PayGate_payment' ),
        'type'    => 'text',
        'desc_tip'  => __( 'ID de clave de seguridad.', 'PayGate_payment' ),
        'default' => '',
      ),
      //api key para el hash de pago
      'api_key' => array(
        'title'   => __( 'Api key', 'PayGate_payment' ),
        'type'    => 'text',
        'desc_tip'  => __( 'ID de clave de api', 'PayGate_payment' ),
        'default' => '',
      ),
      */
  ...
  public function process_payment( $order_id ) {
    ...
    //$environment_url = 'https://PayGate/payment_Gateway.com/';
    //$time = time();
    //$key_id = $this->key_id;
    ...
    $payload = array(
     ...
     "x_card_num"             => str_replace( array(' ', '-' ), '', $_POST['PayGate_payment_gateway-card-number'] ),
     "x_card_code"            => ( isset( $_POST['PayGate_payment_gateway-card-cvc'] ) ) ? $_POST['PayGate_payment_gateway-card-cvc'] : '',
     "x_exp_date"             => str_replace( array( '/', ' '), '', $_POST['PayGate_payment_gateway-card-expiry'] ),
     ...
     // Informacion de Facturacion (billing)
    ...
    $response = wp_remote_post( $environment_url, array(
      'method'    => 'POST',
      //http buils query crea una consulta de tipo http de modo que usara el hash de entrada y contruira una url respectiva
    ...
    if ( is_wp_error( $response ) )
      throw new Exception( __( 'Ooh! Parece que ha ocurrido un problema con la transaccion.', 'PayGate-payment-gateway' ) );
    if ( empty( $response['body'] ) )
      throw new Exception( __( 'No hay respuesta', 'PayGate-payment-gateway' ) );
    ...
    if ( ($r['response_code'] == 1 ) || ( $r['response_code'] == 4 ) ) {
      ...
      $customer_order->add_order_note( __( 'PayGate: Pago completado con exito!.', 'PayGate-payment-gateway' ) );
  ...
  public function validate_fields() {
    return true; //retornamos para activar la validacion
  }
  ...
  public function do_ssl_check() {
    if( $this->enabled == "yes" ) {
      if( get_option( 'woocommerce_force_ssl_checkout' ) == "no" ) {
        ...
        $this->method_title, admin_url( 'admin.php?page=wc-settings&tab=checkout' ) ) ."</p></div>";
      }
    }
  }
}
